...
### v2.0.12
- v2.0.11's release still failed: with the tag correctly read as annotated, `gh release create` succeeded but the workflow's very next check for that release -- a paginated scan of the list endpoint -- came back empty, since that list lags slightly behind its own writes. It now reads the release straight back from the URL `gh release create` printed instead of re-searching for it. No application changes since v2.0.10, whose entry below is what actually shipped
